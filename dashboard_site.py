...
def _series_by_company(df: pd.DataFrame, metric: str) -> dict:
    ...
    for name, group in df.groupby("competitor_name"):
        ...
        datasets.append({"label": str(name or "Без названия"), "data": [round(_num(by_date.get(d)), 2) for d in dates]})
    return {"labels": dates, "datasets": datasets}


def _category_series(cat_df: pd.DataFrame, category_code: str, metric: str) -> dict:
    if cat_df.empty or "category_code" not in cat_df.columns:
        return {"labels": [], "datasets": []}
    return _series_by_company(cat_df.loc[cat_df["category_code"] == category_code].copy(), metric)


def _market_share(latest: pd.DataFrame) -> list[dict]:
    if latest.empty:
        return []
    total = _num(latest["total_area"].sum())
    if total <= 0:
        return []
    rows = []
    for _, row in latest.sort_values("total_area", ascending=False).iterrows():
        area = _num(row.get("total_area"))
        rows.append({
...
def _changes(df: pd.DataFrame) -> list[dict]:
    if df.empty or "competitor_code" not in df.columns:
        return []
    df = df.copy()
    ...
    for _, group in df.groupby("competitor_code"):
        ...
        for metric, title, threshold in [("total_area", "Площадь", 20), ("count", "Помещения", 20), ("avg_price", "Ставка", 15)]:
            old = _num(prev.get(metric))
            new = _num(cur.get(metric))
            if old <= 0:
                continue
...
def build_payload() -> dict:
    ...
    categories = _read_csv("category_history.csv")
    latest = _latest_by_company(history)
    if latest.empty:
        total_count = total_area = total_price = avg_price = own_share = 0
        competitors_count = 0
    else:
        total_count = int(_num(latest["count"].sum()))
        total_area = round(_num(latest["total_area"].sum()), 2)
        total_price = round(_num(latest["total_price"].sum()), 2)
        ...
        own_area = _num(own["total_area"].sum()) if not own.empty else 0
        own_share = round(own_area / total_area * 100, 1) if total_area > 0 else 0
        competitors_count = int(len(latest.loc[latest.get("entity_role", "") != "own_company"]))
    return {
        "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "cards": {"total_count": total_count, "total_area": total_area, "avg_price": avg_price, "total_price": total_price, "own_share": own_share, "competitors": competitors_count},
        "latest": latest.fillna("").to_dict("records") if not latest.empty else [],
        "market_share": _market_share(latest),
        "top_area": latest.sort_values("total_area", ascending=False).head(8).fillna("").to_dict("records") if not latest.empty else [],
        "changes": _changes(history),
        "charts": {
            "area": _series_by_company(history, "total_area"),
            "price": _series_by_company(history, "avg_price"),
            "count": _series_by_company(history, "count"),
            "commercial_area": _category_series(categories, "commercial", "total_area"),
            "industrial_area": _category_series(categories, "industrial", "total_area"),
        },
    }


def build_html() -> str:
    data = json.dumps(build_payload(), ensure_ascii=False)
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if token:
            ...
            if (query.get("key") or [""])[0] != token:
                self._send(403, "Доступ закрыт. Нужна корректная ссылка с ключом.".encode("utf-8"), "text/plain; charset=utf-8")
                return
        self._send(200, build_html().encode("utf-8"))
